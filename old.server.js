...
async function ssrfProtect(req, res, next) {
  ...
  try {
    ...
    if (isPrivateIP(address)) {
      ...
      return res
        .status(403)
        .json({ error: "Internal networks are restricted." });
    }
    next();
  } catch (err) {
    return res
      .status(400)
      .json({ error: "Invalid URL or unresolvable hostname." });
  }
}
...
app.delete("/api/alerts", authenticateToken, async (req, res) => {
  await pool.query(
    "DELETE alerts FROM alerts JOIN sites ON alerts.site_id = sites.id WHERE sites.user_id = ?",
    [req.user.id],
  );
  res.json({ success: true });
});
...
app.delete("/api/alerts/archive", authenticateToken, async (req, res) => {
  await pool.query(
    "DELETE alerts FROM alerts JOIN sites ON alerts.site_id = sites.id WHERE sites.user_id = ? AND alerts.is_read = TRUE",
    [req.user.id],
  );
  res.json({ success: true });
});
...
app.delete("/api/alerts/:id", authenticateToken, async (req, res) => {
  await pool.query(
    "DELETE alerts FROM alerts JOIN sites ON alerts.site_id = sites.id WHERE alerts.id = ? AND sites.user_id = ?",
    [req.params.id, req.user.id],
  );
  res.json({ success: true });
});
...
app.get("/api/proxy", authenticateToken, ssrfProtect, async (req, res) => {
  ...
  try {
    ...
    $("body").prepend(
      `<div id="inspector-overlay"><div id="inspector-label"></div></div><script id="piq-inspector-script">${inspectorJS}</script>`,
    );
  ...
  try {
    ...
    for (let site of sites) {
      ...
      try {
        ...
        await context.route("**/*.{mp4,webm,ogg,mp3,wav}", (route) =>
          route.abort(),
        );
        ...
        const mediaPromises = mediaElements.map(async (el) => {
          ...
          if (rawSrc.startsWith("data:"))
            return `|B64:${crypto.createHash("md5").update(rawSrc).digest("hex")}`;
        ...
        if (site.last_hash && newHash !== site.last_hash) {
          ...
          target
            ...
            .each((i, el) => {
              if (el.attribs.href)
                el.attribs.href = new URL(el.attribs.href, site.url).href;
            });
        ...
        await pool.query(
          ...
          [errCount, err.message.substring(0, 250), site.id],
        );
      }
    }
  } catch (err) {
